...
export default async function CaptionExamplesPage() {
    ...
    const { data, error } = await supabase.from("caption_examples").select("*");

    if (error) {
        return <main className="p-10 text-[#1a3a5c]">Error loading caption examples: {error.message}</main>;
    }
        ...
        <main className="p-12">
            <div className="mx-auto max-w-7xl">
                <div className="mb-6 flex items-center justify-between">
                    <h1 className="text-4xl font-bold text-[#0c1a2e]">Caption Examples</h1>
                    <Link
                    ...
                    <table className="min-w-full text-sm text-[#1a3a5c]">
                        ...
                        {rows?.map((row) => (
                            <tr key={row.id} className="hover:bg-blue-50/50">
                                <td className="px-6 py-4 font-mono text-xs text-[#6a9cbf]">{row.id}</td>
                                <td className="px-6 py-4">{row.caption ?? "-"}</td>
                                <td className="px-6 py-4">{row.example ?? "-"}</td>
                                <td className="px-6 py-4">
                                    <div className="flex gap-4">
                                        <Link href={`/admin/caption-examples/edit/${row.id}`} className="text-[#3b82f6] hover:underline">Edit</Link>
