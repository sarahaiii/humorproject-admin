...
export default async function WhitelistEmailsPage() {
    ...
    if (error) {
        return <main className="p-10 text-[#1a3a5c]">Error loading whitelist emails: {error.message}</main>;
    }

    const rows = data as WhitelistEmailRow[] | null;

    return (
        <main className="p-12">
            <div className="mx-auto max-w-7xl">
                <div className="mb-6 flex items-center justify-between">
                    <h1 className="text-4xl font-bold text-[#0c1a2e]">Whitelisted Emails</h1>
                    <Link
                            ...
                            <tr key={row.id} className="hover:bg-blue-50/50">
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex gap-4">
                                        <Link href={`/admin/whitelist-emails/edit/${row.id}`} className="text-[#3b82f6] hover:underline">Edit</Link>
                                        <form action={`/admin/whitelist-emails/delete/${row.id}`}>
